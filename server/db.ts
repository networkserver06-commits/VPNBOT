...
export async function upsertTelegramSubscriber(input: { telegramUserId: string; username?: string; firstName?: string }) { const { db } = await getMongo(); const result = await db.collection("telegramSubscribers").updateOne({ telegramUserId: input.telegramUserId }, { $set: { username: input.username ?? null, firstName: input.firstName ?? null, lastSeenAt: new Date() } }, { upsert: true }); return Boolean(result.upsertedId); }
export async function listTelegramSubscribers() { const { db } = await getMongo(); return db.collection<{ telegramUserId: string }>("telegramSubscribers").find({}, { projection: { telegramUserId: 1 } }).toArray(); }
